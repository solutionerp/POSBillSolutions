...
namespace POSBill.Domain.Models
{
    ...
    { 
        ...
        {
            return "SELECT * FROM reference_db.0_sys_prefs where name = 'coy_logo'";
        }
        public override string GetUpdateQuery(string id)
        {
            _imagesource = _imagesource.Replace("\\", "\\\\");
            return "Update 0_sys_prefs set value = '" + _imagesource + "'" + " where name = 'coy_logo'";
        } 
        public override string GetInsertQuery()
        {
            return "insert into 0_settings " + "(companylogo) " + "Values " + "('" +_imagesource+"')";
        }
